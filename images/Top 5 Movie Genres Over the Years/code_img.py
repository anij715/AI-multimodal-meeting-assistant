import pandas as pd
import altair as alt
import matplotlib.pyplot as plt
import seaborn as sns
import ast
import requests
from io import StringIO # StringIO is used for text-based in-memory "files"

csv_url = "https://mailuc-my.sharepoint.com/:x:/g/personal/sharmarz_mail_uc_edu/Ef5POYmr8M1PoXaiJbh-QlEBLkKKf4K8AHwhm--qsSnixA?download=1"

headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
}

response = requests.get(csv_url, headers=headers, timeout=90)
response.raise_for_status()
try:
    csv_text_content = response.content.decode('utf-8')
    print("Successfully decoded content as UTF-8.")
except UnicodeDecodeError:
    print("UTF-8 decoding failed. Trying 'latin1' (iso-8859-1)...")
    try:
        csv_text_content = response.content.decode('latin1')
        print("Successfully decoded content as 'latin1'.")
    except Exception as decode_err:
        print(f"Could not decode file content. Please check the file's encoding. Error: {decode_err}")
        raise # Re-raise the exception if decoding fails

movies = pd.read_csv(StringIO(csv_text_content), dtype={10: str})
movies['revenue'] = pd.to_numeric(movies['revenue'], errors='coerce')
movies['budget'] = pd.to_numeric(movies['budget'], errors='coerce')
movies['release_date'] = pd.to_datetime(movies['release_date'], errors='coerce')

cleaned_movies = movies.dropna(subset=['revenue', 'budget', 'release_date'])

cleaned_movies = cleaned_movies[(cleaned_movies['revenue'] > 0) & (cleaned_movies['budget'] > 0)]


import ast

# Parse the genres column (it contains lists as strings)
def parse_genres(genres_str):
    try:
        genres_list = ast.literal_eval(genres_str)
        return [genre['name'] for genre in genres_list]
    except (ValueError, TypeError):
        return []

cleaned_movies['parsed_genres'] = cleaned_movies['genres'].apply(parse_genres)
cleaned_movies['release_year'] = cleaned_movies['release_date'].dt.year
avg_revenue_per_year = cleaned_movies.groupby('release_year')['revenue'].mean()

# Count the frequency of each genre
from collections import Counter

genre_counter = Counter(genre for genres in cleaned_movies['parsed_genres'] for genre in genres)
genres, counts = zip(*genre_counter.items())

# Exploding the genres into multiple rows for detailed analysis
genres_exploded = cleaned_movies.explode('parsed_genres')
genres_per_year = genres_exploded.groupby(['release_year', 'parsed_genres']).size().unstack(fill_value=0)

# Select top genres overall for clear visualization
top_genres = genre_counter.most_common(5)
top_genres_list = [genre[0] for genre in top_genres]

# Filter the data to include only the top genres
top_genres_per_year = genres_per_year[top_genres_list]


top_genres_per_year.plot(kind='line', marker='o', linewidth=2, figsize=(14, 8), colormap='tab10')
plt.title("Trends of Top 5 Movie Genres Over the Years")
plt.xlabel("Year")
plt.ylabel("Number of Movies")
plt.legend(title="Genres")
plt.grid(True)
plt.tight_layout()
plt.show()