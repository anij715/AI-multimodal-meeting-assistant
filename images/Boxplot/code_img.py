...
genre_medians = genres_exploded.groupby("parsed_genres")["revenue"].median().sort_values()

# Reset index to avoid duplicate index issue and sort by median revenue
sorted_data = genres_exploded.set_index("parsed_genres").loc[genre_medians.index].reset_index()

# Plot sorted box plot
sns.boxplot(data=sorted_data, x="parsed_genres", y="revenue", showfliers=False, order=genre_medians.index)

plt.yscale("log")  # Scale to show better spread
plt.title("Revenue Distribution by Genre (Sorted by Median)")
plt.xlabel("Genres")
plt.ylabel("Revenue (Log Scale) - USD")
